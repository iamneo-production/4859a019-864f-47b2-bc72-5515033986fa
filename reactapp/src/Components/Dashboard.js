...
export default function Dashboard() {
    ...
    useEffect(()=>{
        axios.get("http://localhost:51700/admin")
        .then(res => {setProductData(res.data)})
        .catch(err => {console.log(err)});
    },[count])
    function editproduct(event,index){
        
        axios.get("http://localhost:51700/admin/productEdit/"+productData[index].id).then(res=>{
    ...
    function deleteproduct(event,index){
        ...
        axios.delete("http://localhost:51700/admin/delete/"+id)
        .then(res=>{
            alert(res.data);
            setcount(count+1);
        })
  ...
  return (
    <>
        ...
        <a className= "btn btn-primary mb-2" href="/addProduct">ADD A PRODUCT</a>
        <div className='container-fluid' id="addProductBody">
            <div className='row' style={{'color':'white','padding':'10px',backgroundColor:'#777',borderTopLeftRadius:'8px',borderTopRightRadius:'8px'}}>
                ...
                <div className='col-lg-2'>ProductName</div>
            ...
            {productData.map((productobj,index)=>{return(
                <div className='row' key={index} style={{'color':'white','padding':'10px',backgroundColor:'#495057'}}>
                    <div className='col-lg-4'><img src={productobj.imageurl} alt="" style={{'width':'100px','height':'100px'}}></img></div>
                    <div className='col-lg-2'>{productobj.productName}</div>
